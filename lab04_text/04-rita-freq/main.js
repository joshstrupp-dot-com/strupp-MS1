...
// our input text
const textData = `Mr. Speaker, Mr. Vice President, Members of Congress, the First Lady of the United States, and my fellow Americans:
...
function analyzeText(textData) {
  ...
  words.forEach((word) => {
    word = word.toLowerCase().replace(/[^\w\s]/gi, ""); // Normalize words
    if (word) {
      const pos = RiTa.getPosTags(word)[0];
      if (pos === "nn" || pos === "vb" || pos === "jj") {
        // Noun, Verb, Adjective
  ...
  const SOTU_WordCounts = {};
  sortedWordCounts.forEach(([word, count]) => {
    SOTU_WordCounts[word] = count;
    console.log(`${word}: ${count}`);
  });

  return SOTU_WordCounts;
}

const SOTU_WordCounts = analyzeText(textData);

// document.body.innerHTML = document.body.innerHTML.replace(
//   /\bamerican\b/gi,
//   (match) => `${match}<p style="display:inline; font-size:small;"> (29)</p>`
// );

//console log the word count for "Speaker"
console.log(SOTU_WordCounts["american"]);

//for all instances of "American" in the text, add a small font size count of the word count
document.body.innerHTML = document.body.innerHTML.replace(
  /\bamerican\b/gi,
  (match) =>
    `${match}<p style="display:inline; font-size:small;"> (${SOTU_WordCounts["american"]})</p>`
);

document.body.innerHTML = document.body.innerHTML.replace(
  /\bspeaker\b/gi,
  (match) =>
    `${match}<p style="display:inline; font-size:small;"> (${SOTU_WordCounts["speaker"]})</p>`
);
